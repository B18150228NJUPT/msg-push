...
import static com.ycc.msgpush.threadPool.ThreadPoolExecutorManager.holder;
...
//@ConditionalOnBean(value = ThreadPoolExecutorManager.class)
@Slf4j
public class ReceiverStart {
     ...
     * 获取得到所有的groupId
     */
    private static List<String> groupIds = Lists.newArrayList(holder.keySet());
    /**
     * 下标(用于迭代groupIds位置)
     */
        ...
        factory.setRecordFilterStrategy(consumerRecord -> {
            if (Optional.ofNullable(consumerRecord.value()).isPresent()) {
                for (Header header : consumerRecord.headers()) {
                    if (header.key().equals("tagId") && new String(header.value()).equals(new String(tagIdValue.getBytes(StandardCharsets.UTF_8)))) {
                        return false;
                    }
                }
            }
